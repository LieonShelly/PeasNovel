...
extension ObservableConvertibleType where Element == Response {
    
    func logPrinter() -> Observable<Element> {
        return asObservable()
            .map{
                $0.logPrinter()
                return $0
    ...
    func model<T: HandyJSON>(_ type: T.Type) throws -> T {
        ...
        guard let rawJson = try? JSONSerialization.jsonObject(with: data, options: .allowFragments) as? [String: Any],
            ...
            let code = status["code"] as? Int  else  {
                let testrawJson = try? JSONSerialization.jsonObject(with: data, options: .allowFragments)
                print("[URL] - MoyaError\t:", self.request?.urlRequest?.url ?? "")
                print("MoyaError:\(testrawJson ?? "")")
...
//        print("[RES]\t\t:",rawJson ?? "", "\n")
    ...
    func fullModel<T: HandyJSON>(_ type: T.Type) throws -> T {
        print("✈ -------------------------------------------- ✈")
        print("[URL]\t:", self.request?.urlRequest?.url ?? "")
        if let paramData = request?.urlRequest?.httpBody {
            do{
        ...
        guard let rawJson = try? JSONSerialization.jsonObject(with: data, options: .allowFragments) as? [String: Any],
            let result = rawJson?["result"] as? [String: Any] else  {
                throw MoyaError.jsonMapping(self)
        }
//        print("[RES]\t\t:",rawJson ?? "", "\n")
        guard let ret = JSONDeserializer<T>.deserializeFrom(dict: result) else {
